...
public class vShooterMeleeInput : vMeleeCombatInput
{
    ...
    void ShotInput()
    {
        ...
        if (isAiming && !shooterManager.shooting && aimConditions && !isReloading && !isAttacking)
        {
    ...
    protected virtual void UpdateShooterAnimations()
    {
        ...
        // character is aiming
        animator.SetBool("IsAiming", isAiming && !isAttacking);
    ...
    protected virtual void UpdateLeftIK()
    {
        ...
        if (leftIK != null)
        {
            ...
            if (shooterManager.rWeapon && shooterManager.rWeapon.handIKTarget && Time.timeScale > 0 && !isReloading && !cc.actions && !cc.customAction && !isEquipping && (cc.isGrounded || isAiming) && !cc.lockMovement && useIkConditions)
    ...
    protected virtual void RotateRightArm()
    {
        if (shooterManager.rWeapon && isAiming && aimConditions && shooterManager.rWeapon.alignRightUpperArmToAim)
        {
            ...
            rightRotationWeight = Mathf.Lerp(rightRotationWeight, !shooterManager.shooting || shooterManager.rWeapon.ammoCount <= 0 ? 1f * aimWeight : 0f, 10f * Time.deltaTime);
